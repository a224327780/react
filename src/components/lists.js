...
import {getProgressColor} from "../utils/util";
import useSWR from "swr";

const Items = () => {
    // const [items, setLists] = useState([]);
    const { data, error } = useSWR('https://nodequery-atlinbing.cloud.okteto.net/lists')
    console.log(data)
    console.log(error)
    // useEffect(() => {
    //     setLists(data['data'])
                    ...
                    <Link to={`/${item._id}`}>
                        <div className="flex mb-2 ">
                            <div className="flex flex-auto items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${item.is_online} ? 'text-green-500' : 'text-red-500'}` }
                                     viewBox="0 0 20 20"
                                     fill="currentColor">
                        ...
                        <div className="flex text-gray-500 text-sm mb-6">
                            <div className="flex flex-auto items-center ">1</div>
                        ...
                        <div className="mt-2 flex space-x-4">
                            ...
                            <div className="flex-auto flex items-center ">
                                <div className="w-full rounded bg-gray-200 h-4">
                                    <div style={{width: `${item.load}%`}} className={getProgressColor(item.load)}></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-2 flex space-x-4">
                            ...
                            <div className="flex-auto flex items-center ">
                                <div className="w-full rounded bg-gray-200 h-4">
                                    <div style={{width: `${item.ram}%`}} className={getProgressColor(item.ram)}></div>
                                </div>
                            </div>
                        </div>
                        <div className="mt-2 flex space-x-4">
                            ...
                            <div className="flex-auto flex items-center">
                                <div className="w-full rounded bg-gray-200 h-4">
                                    <div style={{width: `${item.disk}%`}} className={getProgressColor(item.disk)}></div>
                                </div>
                            </div>
                        </div>
                    </Link>
